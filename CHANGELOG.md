...
- Jorvik: gear/materials/props pickups use `world/pickups/*_pickup.tscn` wrappers; added `wood_planks` material and aligned `pickup_scene_path` on affected items (torch pickup scene is now the gltf wrapper, not `torch_light.tscn`).
...
- Jorvik and environment updates; sky/day-night tuning.
